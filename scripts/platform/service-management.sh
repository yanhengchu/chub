...
# These units were installed by retired Chub releases and are no longer
# generated by this project. Keep the list explicit: uninstall must not
# wildcard-delete a maintainer's unrelated user services.
RETIRED_SYSTEMD_SERVICE_NAMES=("chub-loopback-forwarder")
...
uninstall_all_services() {
    case "$PLATFORM" in
        ...
        Linux)
            ...
            local retired_service
            for retired_service in "${RETIRED_SYSTEMD_SERVICE_NAMES[@]}"; do
                systemctl --user disable --now "$retired_service.service" >/dev/null 2>&1 || true
            done
            ...
            for retired_service in "${RETIRED_SYSTEMD_SERVICE_NAMES[@]}"; do
                rm -f "$SYSTEMD_DIR/$retired_service.service"
            done
            systemctl --user daemon-reload
            ;;
        *) fail "unsupported platform: $PLATFORM" ;;
