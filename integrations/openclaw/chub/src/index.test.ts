...
function createPluginApi(pluginConfig: Record<string, unknown> = {}) {
  ...
  return { hooks, registerTool: api.registerTool };
}

describe("plugin boundary", () => {
  it("registers only the Weixin forwarding hook", () => {
    const { hooks, registerTool } = createPluginApi();

    expect([...hooks.keys()]).toEqual(["before_dispatch"]);
