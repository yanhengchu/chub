...
> 状态：统一消息调度 v3 已部署；普通任务即时提交回执已通过自动化验证和维护者真实微信验收。
...
插件只把可信语音来源归一化为干净转写和 `voice` 类型。文字和语音任务的即时回执由 Chub 按“状态、可信 Session、Task 摘要”生成；Session 固定在 Task 前，`▶` 只标记当前绑定，目标或归属无法可靠确认时省略 Session。明确失败使用相同层级。固定路由文案同样全部由 Chub 决定，插件不得追加、删减或改写。插件等待 Chub 超时时只能说明当前提交状态未知并提示不要重复发送，不能据此宣称任务提交失败，也不在插件侧生成 Session 或摘要。
...
- 普通任务明确受理时即时返回包含状态、可信 Session 和 Task 摘要的紧凑提交回执；明确拒绝时即时返回失败原因；请求超时时只报告提交状态未知并阻止用户盲目重发。
- 主任务成功通知在最后一段底部追加 `Weekly <quota> · Today <usage>`；额度不可用时显示 `Weekly Unavailable`，失败、超时和翻译通知不追加，额度采集不得改变任务结果或阻断通知。
