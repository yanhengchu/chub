...
> 状态：统一消息调度 v3 已完成仓库实现和自动化验证。验收基线方面，普通文本、可信语音、同步提交状态和异步最终结果已在 macOS、Ubuntu 验收，任务状态检查路由由维护者暂按验收通过；当前部署方面，v3 已在 macOS 生效，Ubuntu 使用本次版本前仍需同步部署 v3。后续根据实际体验持续优化。
...
3. 固定任务状态短语进入 Chub 状态检查路由；运行中直接回复，已结束未通知或通知失败时触发原路通知。
4. 其他非空业务正文按现有安全边界创建快速交互。
5. 保存需要异步完成的任务、微信路由和最终通知状态。

当前接口只识别能力清单登记的固定状态检查短语，不接受任务编号、Session 或动作参数；其他非空业务正文进入普通任务提交流程。
...
- 重复消息不重复创建任务。
- 四个固定任务状态短语只进入状态检查路由，近似文本仍按普通任务提交。
- 运行中任务直接回复状态；通知失败任务后台登记原路通知并返回 `handled`，插件不产生额外回复。
- 状态检查期间重启会把未完成预留恢复为明确失败，同一消息不会重复触发通知。
- 原路通知耗时超过统一调度等待窗口时，状态检查仍能立即结束，最终状态由后台通知记录。
- 最终结果、超长结果、部分送达、Context Token 失效和重新入站刷新均保持受控。
- `chub_get_status`、`chub_send_notification` 与飞书原文保护没有退化。

验收基线：普通文本和可信语音链路已在 macOS、Ubuntu 完成真实验收，新增任务状态检查路由由维护者暂按验收通过。部署状态：v3 已在 macOS 完成插件部署和通道加载检查；Ubuntu 使用本次版本前需同步部署 v3，并确认 Chub 与插件版本一致。后续如在实际微信体验中发现匹配、回复或通知重试问题，再按现象补充回归和调整。版本不一致期间按失败关闭处理。
