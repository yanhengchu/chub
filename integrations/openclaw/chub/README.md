...
> 状态：统一消息调度 v3 已在 macOS、Ubuntu 完成部署和核心链路验收。Chub 状态总览、槽位维护和 Codex 明确操作均由 Chub 在现有统一接口内处理，不改变插件协议或业务逻辑；本次 Chub 侧路由调整尚待当前节点真实微信验收，Ubuntu 尚未同步。
...
- 普通文字和可信语音只调用一次统一调度接口并成功提交任务；普通文字成功提交由 Chub 决定静默处理。
- Chub 返回的静默、失败和语音回显决定均由插件原样执行，插件不再拼装业务回复。
- 重复消息不重复创建任务。
...
验收基线：普通文本、可信语音和 v3 插件已在 macOS、Ubuntu 完成真实验收。`chub` 缓存总览、`chub refresh`、`chub sync`、中文别名、裸 `codex` 普通提交、`切换N` 和单步 `归档N` 已完成 Chub 自动化验证且不涉及插件协议变化，因此不需要重新构建或部署插件；Chub 同步重启后仍需分别用微信文字和语音按能力清单验收，确认缓存时间、系统告警、任务计数、兼容 Session、`[Current]` 标记、显式补槽、中文数字切换、归档后的槽位释放，以及重启回送中的独立 Codex Usage 和 Active sessions。协议版本不一致期间按失败关闭处理。
