...
async def settings_page_removes_quick_interaction_page_size_preference(
    ...
    assert "loadRuntimeManagement" not in script.text
    assert "saveRuntimeEnablement" not in script.text
...
@pytest.mark.anyio
async def test_settings_navigation_hides_unimported_orchestration_plugin(
    settings: Settings,
) -> None:
    ...
    assert response.status_code == 200
    assert '<span class="settings-navigation-subgroup">任务编排</span>' not in response.text
    assert 'class="settings-navigation-link settings-navigation-child" href="/settings/task-orchestration"' not in response.text
...
async def test_settings_pages_use_independent_routes_and_page_scoped_content(
    ...
    assert "统一管理导入、移除、启用和禁用" not in pages["runtime"].text
    assert 'id="plugin-lifecycle-list" class="settings-divided-list runtime-module-list"' in pages["runtime"].text
    ...
    assert '.deliveryline-version-actions {' in stylesheet.text
    ...
    assert 'id="runtime-management-status"' not in pages["runtime-detail"].text
    assert 'id="runtime-management-message"' not in pages["runtime-detail"].text
    assert 'name.textContent = "Runtime 状态";' not in script.text
    assert 'input.id = `runtime-enabled-${runtime.runtime_id}`;' not in script.text
    assert 'control.htmlFor = input.id;' not in script.text
    assert 'moduleEmptyRow("尚未导入 Runtime 插件。")' not in script.text
    assert 'moduleEmptyRow("尚未导入任务编排插件。")' not in script.text
    assert 'formalImplementationTitle(module.name || "Runtime", module.version)' not in script.text
    assert 'badge.textContent = module.status === "active" ? "可用" : "不可用";' not in script.text
    assert 'function refreshPluginManagementNavigation() {' not in script.text
    assert 'if (settingsPage === "runtime") window.location.reload();' not in script.text
    assert 'badge.textContent = module.active ? "当前使用" : (module.available ? "可用" : "不可用");' not in script.text
    assert 'formalImplementationTitle(module.name || "任务编排插件", module.version)' not in script.text
...
async def test_runtime_settings_navigation_lists_each_registered_runtime(
    ...
    assert 'data-runtime-id="local"' not in local.text
...
async def test_deliveryline_workspace_section_follows_import_lifecycle(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        unavailable_page = await client.get("/?section=deliveryline")
        imported = await client.post(
        ...
        removed_home = await client.get("/")
        removed_page = await client.get("/?section=deliveryline")

    assert initial_home.status_code == 200
    assert unavailable_page.status_code == 404
    assert 'aria-label="业务导航"' not in initial_home.text
    assert imported.status_code == 200
    assert 'aria-label="业务导航"' not in imported_home.text
    assert disabled_page.status_code == 404
    assert disabled_api.status_code == 409
    assert disabled_api.json()["error"]["code"] == "deliveryline_not_enabled"
...
@pytest.mark.anyio
async def test_deliveryline_api_rejects_an_enabled_but_unavailable_plugin(
    settings: Settings,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    application = create_app(settings)
    lifecycle_list = application.state.plugin_lifecycle.list
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        def unavailable(request):
            payload = lifecycle_list(request)
            plugin = next(item for item in payload["plugins"] if item["plugin_id"] == "deliveryline")
            artifact = next(item for item in plugin["artifacts"] if item["artifact_id"] == "development:deliveryline")
            artifact["available"] = False
            artifact["reason"] = "模拟不可用"
            return payload
    ...
    assert rejected.json()["error"]["code"] == "deliveryline_unavailable"
...
async def test_automation_section_uses_workstation_status_rows(
    ...
    assert 'automationCodexAccountDetail.dataset.accountState = state.state;' in workspace_script.text
    assert 'const accountCheckInProgress = state?.state === "checking";' in workspace_script.text
    assert 'accountCheckInProgress && !automationCodexAuthSwitching' in workspace_script.text
    assert 'automationCodexAccountDetail?.dataset.accountState === "checking"' in workspace_script.text
    ...
    assert "正式周报已生成，请查看" in stale_report_response.text
    assert 'href="/weekly-reports/2026-08-31%E8%87%B32026-09-06/report">查看文档</a>' in stale_report_response.text
    assert ">重新运行</button>" in stale_report_response.text
    assert ">确认并生成正式周报</button>" not in stale_report_response.text
...
async def test_quick_interaction_conversation_page_is_available(
    ...
    assert '.workspace-session-create-description,' in stylesheet.text
    assert 'workspace-session-create-field-group' not in page.text
...
async def test_design_document_pages_render_markdown(settings: Settings) -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        removed_business_module = await client.get("/project-docs/chub-business-module")
    ...
    assert listing.text.index('href="/project-docs/chub-architecture"') < listing.text.index(
...
async def test_weekly_report_detail_is_public_and_missing_report_is_404(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        focus_template = await client.get("/weekly-reports/templates/focus")
        report_template = await client.get("/weekly-reports/templates/report")
