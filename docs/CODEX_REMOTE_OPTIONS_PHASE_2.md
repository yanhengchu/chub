...
2026-07-21 已完成 macOS、Ubuntu 和手机端验收：会话新建、进入、停止、恢复、归档和删除正常；长输出可滚动查看；浏览器退出和跨设备接管不会停止本机 Codex；新设备进入后旧页面会自动返回节点页；节点维护、跨平台运行和移动端布局符合当前使用要求。

## 4. 结论

PTY 方案已经通过当前阶段三端验收：手机可操作 macOS 与 Ubuntu 节点上的原生 Codex TUI，且能保留会话、恢复和安全边界。PTY 方向正式收尾，后续只需完成 M2 网络可达性验收；remote-control 与 app-server 继续保留为备选方案。
