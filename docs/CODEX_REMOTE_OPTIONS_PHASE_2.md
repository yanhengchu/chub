...
- 同一 session 不允许与活动或正在建立的 ttyd WebSocket、其他快速交互任务并发执行；实时会话正在执行时也拒绝切换，避免多个 Codex 进程同时写入同一会话。只有实时会话空闲且没有终端连接时，才允许自动停止 ttyd/tmux 后启动快速交互。
...
默认展示最新 3 条，底部“加载更多”每次继续读取 3 条；历史仍保存在 Chub 本地，全局最多保留
30 条快速交互任务，接口使用有界分页，不一次返回全部结果。页面只在存在运行中任务且可见时轮询，
使用当前标签并依赖浏览器返回，不增加专用返回按钮。
...
会话尚未启动、实时会话正在执行、终端连接仍占用或同一 session 已有快速交互任务时拒绝提交；
空闲且无连接的实时会话会自动停止后切换。快速交互超时为
...
首页不把快速交互复用为 PTY 的 `activity`，而是在 session 列表中独立返回
`quick_interaction_running` 和对应更新时间。后台内部短暂的 `requested` 不作为产品状态展示；
从提交被接受到任务成功、失败或超时结束，首页统一显示“快速交互 · 执行中”，结束后恢复原有的
“会话已停止 · 可恢复”。执行期间禁用重复提交、进入实时终端、归档和删除；前后端使用同一
session 锁避免检查与操作竞态，“交互记录”仍可正常查看。活动中的快速交互状态优先于历史
PTY 错误展示，任务结束时同步 session 更新时间，避免首页恢复基础状态后时间倒退。

首页继续只轮询 `GET /api/codex/sessions`：存在快速交互执行中，或存在 PTY
`running + working/unknown` 时继续轮询；快速交互执行期间固定使用快速轮询，页面隐藏时暂停，
重新显示时立即刷新，所有活动状态结束后停止。快速交互提交成功后立即定向刷新 Codex 卡片，
不为每个 session 额外查询任务列表。交互记录页仍独立轮询任务详情，终端页仍使用 WebSocket。

快速交互与实时终端只在实际执行阶段互斥。会话为 `running + idle` 且没有活动或正在建立的终端
WebSocket 时，提交快速交互会在同一 session 锁内自动停止 ttyd/tmux，再启动
`codex exec resume`；实时会话为
`working` 或仍有终端 WebSocket 占用时拒绝切换。已 claim 但尚未完成后端连接的 WebSocket 也
视为占用；session 关闭后该 claim 不能再次激活。服务重启后状态为 `unknown` 时，页面要求用户明确
确认可能中断任务；警告已经显示时，点击“执行”本身即表示确认，不增加额外勾选项。若弹窗打开后
后端状态才变为 `unknown`，第一次提交只在原弹窗中补充警告，用户再次点击后才携带确认。后端收到
