...
Chub 同时是插件模块的本机宿主：核心提供工作台壳、可信入口、模块生命周期、稳定能力和最终状态确认；插件模块提供自身的业务页面、设置、数据与流程。模块不是第四个业务层，也不能反向成为核心依赖。“插件模块”是统一架构术语；正式交付和安装形态统一称为“插件包”或“插件 ZIP”。当前 Runtime 插件模块与微信任务编排插件模块已实现；通用工作台业务插件模块属于后续目标，必须另行定义页面挂载、设置登记、数据归属和 ZIP 协议后才能接入。
...
插件模块分为 Runtime、任务编排和未来工作台业务模块三类。它们使用各自的 ZIP 协议、安装目录、注册表和恢复规则，不能互相借用模块类型或扩大为通用插件市场。业务模块可管理自身的页面、设置、资料、流程和业务记录；需要 AI 执行时，复用 Chub 提供的 Session、任务和状态能力。
