...
Chub 同时是插件模块的本机宿主：核心提供工作台壳、可信入口、统一生命周期和最终状态确认；插件模块提供自身的业务页面、设置、数据与流程。统一层管理配置校验后的导入、移除、启用和禁用状态：导入记录是首页状态和设置侧栏插件菜单的唯一可见性来源，移除后两者同时隐藏；启用记录决定模块业务写入是否可用。插件自身负责加载、运行态确认、回滚与业务扩展动作。模块不是第四个业务层，也不能反向成为核心依赖。Deliveryline 当前已实现“需求提出”的共享档案与评审前校验，不涉及后续阶段或 Chub 执行流程。
