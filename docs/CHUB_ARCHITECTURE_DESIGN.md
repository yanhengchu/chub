...
Chub 同时是插件模块的本机宿主：核心提供工作台壳、可信入口、统一生命周期和最终状态确认；插件模块提供自身的业务页面、设置、数据与流程。统一层管理配置校验后的导入、移除、启用和禁用状态：导入记录是首页状态和设置侧栏插件菜单的唯一可见性来源，移除后两者同时隐藏；启用记录决定模块业务写入是否可用。宿主按模块协议装载入口并确认装配结果；插件提供类型专属入口、运行态确认、回滚与业务扩展动作。模块不是第四个业务层，也不能反向成为核心依赖。Deliveryline 当前确认的是插件生命周期、设置页插件状态和首页入口；仓库中原有的交付线创建与澄清流程属于待重构旧原型，不作为架构层的稳定业务能力。
...
插件模块分为 Runtime、任务编排和工作台业务模块三类。它们使用各自的协议、安装目录、注册表和恢复规则，不能互相借用模块类型或扩大为通用插件市场。内置开发源码统一位于 `modules/`，由 `modules/chub-modules.json` 显式登记；平级 `chub-local-modules/` 是同构的本机模块源，必须使用自己的显式索引，且不属于 Chub 部署、升级或恢复边界。扫描始终先内置、后本机；本机同 ID 项直接忽略。Runtime 模块和任务编排统一分发边界已实现；任务编排插件宿主支持首个插件的索引发现、协议预检、导入登记、开发实现启停以及 Web 装配状态确认，但 ZIP 生命周期和非空阶段插件仍未开放。业务模块的首个固定开发实现是 Deliveryline，源码根目录为 `modules/business/deliveryline/`，其领域模型、存储、协作、业务 API 和模块页面资源均随模块源码维护。当前业务模块宿主管理其受控实现、导入和启用状态，并提供首页入口；交付线创建、整体澄清、交付项和 Chub 执行关联仍处于产品重构范围。

`modules/` 只保存受版本控制的开发模块，不保存已安装 ZIP、任务、缓存或其他运行态。`chub-modules.json` 是内置目录的发现索引；`runtime/` 放实现 Adapter 与 Worker Runner 的 AI Runtime，`orchestration/` 放受限任务编排阶段实现，当前登记 `chub-task-prompt-optimizer` 的开发制品，`business/` 放工作台业务模块的开发制品。每个类型目录可以包含多个模块，实际加载身份始终以索引和各模块 Manifest 为准。平级 `chub-local-modules/` 采用相同的类型目录布局，但只承载当前设备维护者自行登记的代码。
...
| 导入与生命周期 | 提供导入、移除、启用、禁用、按模块协议装载入口、状态查询、操作记录和最终状态确认；导入不自动启用，生命周期变化按模块范围生效 | 提供符合协议的入口与类型专属运行态检查、恢复和业务扩展 |
...
通用移除入口允许维护者对已启用制品发起移除：宿主先持久化停用，阻止后续新请求使用，再清除导入登记及该制品专属安装副本。单纯“已启用”或当前 Web 实例仍持有已装配代码，不单独阻止移除；实例保留旧装配时应明确报告待重载。只有模块专项能力能确认存在不可中断的未终态请求引用时，才保留制品并说明移除受阻原因与恢复路径；新增非空任务阶段前必须提供该引用检查。此过程不得改写已受理请求快照或清理通用设置、其他模块及第三方数据。

当前实现仍是模块化单体。`modules/` 中的开发源码由主项目组合根按模块清单发现，并通过业务模块入口注册 API、工作台页面片段、设置页面片段、静态资源和模块状态钩子；主项目只装配通用壳与宿主能力。正式业务模块 ZIP 导入后由宿主完成清单校验、安全解压和独立安装目录记录；启用状态写入生命周期状态，下一次 Web 启动时按安装目录中的 Manifest 和入口加载模块代码。任务编排插件复用受控索引与通用插件导入/移除入口，按自身 Manifest/协议进行静态预检；开发实现登记或 ZIP 导入只产生未启用的导入记录，ZIP 安装在任务编排专属目录并以内容摘要区分制品。移除开发实现只撤销登记，移除 ZIP 同时清理该制品专属安装副本，两者均保留原始来源。当前任务编排插件的仓库开发实现已支持通用生命周期启停；Web 装配会重新校验源码摘要并调用其最小入口，当前只确认模块描述装配，不注册或执行阶段。ZIP 启停加载与运行时恢复仍待后续交付；提示词优化插件的专属设置页已支持 `direct` / `auto` 配置，默认 `direct`，`auto` 在任务阶段就绪前不可选。当前配置不接入分发链，也不改变已受理任务或普通任务行为。导入不自动启用，启用或停用只影响后续 Web 装配，不改写已受理任务或模块业务数据。
