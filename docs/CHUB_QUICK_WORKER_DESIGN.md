...
- 已交接的快速交互任务、翻译任务和对应 Session 租约：Worker；翻译任务在 Worker 交接前的持久化排队与交接结果：Chub 文本优化状态。
...
- 自动润色和确认模式在翻译条目以原消息 ID、固定目标和回送路由持久化进入 Chub 队列后即视为已受理，微信同步链路立即结束；Worker 的隐藏翻译任务交接在后台进行。只有润色文本对应的主任务也被 Worker 接收后，Web 才进入 `Started` 通知流程。
- 润色中文和 English 各自最多 8000 字符，超限按翻译失败收敛且不得写入不可恢复状态。Web 重启时，尚未开始 Worker 交接的持久化队列项继续按同一操作标识派发；已开始交接但找不到对应 Worker 任务时必须通知失败，不自动重放。派生主任务已经提交时按保存标识幂等确认，不得再次提交或改写为未提交。
...
- 同步提交失败在重启对话框内展示；异步启动失败、超时或健康确认失败通过首页“核心服务”卡的 Chub Web 分区和操作时间线展示。
...
- 当前 macOS 实例已确认 Web/Worker 服务边界、安装维护、活动任务保护、系统升级服务切换和本次协议/终端重连后的最终状态；Ubuntu 保留自动化与服务切换模拟覆盖，尚未在本轮重新实机复检。其他未实际复检的平台不自动获得支持承诺。
