...
普通文本原始正文的首尾空格不属于 Chub 兼容承诺。不得为了保留这类空格修改 OpenClaw 核心、微信适配器或 Chub 插件，也不得把空格替换为斜杠来伪造指令前缀。
...
9. 普通 Gateway/Chub 重启不是补丁触发条件；微信插件升级、重装、实际运行目录变化、运行时代码变化或确认兼容能力缺失时必须复检。
...
文本优化失败时失败关闭：不得执行原文、切换 Session、重复提交或生成虚假的成功状态。语音必须使用可信来源标记和干净转写。
...
3. 版本不匹配、锚点缺失、补丁上下文不一致或构建工具链不可用时停止，不强行修改。
...
补丁应用失败或最终验收失败时，停止继续提交，保留诊断信息，使用与目标版本匹配的官方微信插件副本恢复；不得对版本不匹配的源码盲目执行反向 patch。恢复后重新确认 Gateway、微信通道和 Chub 插件状态。
