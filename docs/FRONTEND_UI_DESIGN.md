...
- Cyber 代码雨随机使用 `0–1` 和由点或空格连接的本地小写短语库；短语覆盖日常、开发和沟通语境，运行时不依赖外部网络。额度雨列只消费共享 AI Usage 快照，将 `weekly`、`today` 显示为单字符垂直雨列，最左轨道持续展示 `weekly`，最右轨道持续展示 `today`；额度来源、缓存、认证失败和不可用规则由[AI 额度与用量采集设计](AI_QUOTA_USAGE_DESIGN.md)维护；
