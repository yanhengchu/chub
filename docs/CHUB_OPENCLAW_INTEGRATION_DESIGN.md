...
> 状态：持续维护。本文描述微信 ClawBot 单入口调度、微信专用任务、完成通知和安全边界，并记录已确认的近期架构调整；具体能力状态与插件部署状态分别以能力清单和插件说明为准。当前 Session 命名与指令路由规则已在 macOS、Ubuntu 完成验收。
...
插件只负责部署开关、微信私聊范围、可信账号与发送者、原始正文、语音来源归一化、幂等标识、固定 Tailnet 地址、超时和有界响应。它不识别业务指令，不检查 Codex 或 Session 状态，不生成或修改任务摘要、提交状态和失败文案，也不让正文指定动作、接口、任务编号、Session、权限、模型、路径或命令。Chub 不可达、协议不匹配或响应无效时，插件只返回统一通道失败，不解释具体业务原因。
...
ClawBot 中的 `chub` 是 Chub 全局状态入口，不经过 Agent 或 LLM。每次查询现场采集系统、当前已分配 Session 和 Codex 用量，并读取本地任务状态；它不扫描或调整槽位，也不触发失败通知补发。`chub refresh`、`chub -f` 和 `刷新状态` 不再属于固定路由。裸 `codex` 不属于固定路由，按普通任务处理；Codex Session 的明确操作指令继续使用固定白名单路由。

状态快照按系统、微信任务、Session 和 Codex 用量分区维护。各分区独立记录最近成功值和采集时间；某一分区失败不能清空或阻塞其他分区。Chub 启动后异步执行一次有界初始化采集，不阻塞服务就绪；初始化期间返回已经取得的部分结果。

任务计数在查询时读取本地持久任务状态，Session 快照由本地 Session 操作更新，并在展示时用当前快速交互运行态覆盖 `Busy`。每次 `chub` 查询并行采集系统状态、当前已分配 Session 和 Codex 用量；并发查询共享正在执行的同一次采集，不设置刷新冷却。启动初始化缓存只用于服务刚启动和采集失败时兜底，不在用户总览中暴露缓存年龄或刷新机制。内存或磁盘达到 85% 时进入总览告警；不使用 CPU 瞬时值触发首版告警。后续调整这些规则时必须同步能力清单和相关测试，不能允许消息正文覆盖。

本地状态必须直接反映能够确定的变化：每次查询从快速交互私有状态汇总任务与通知计数；Session 创建、切换、归档和槽位同步更新 Session 快照，查询时再覆盖正在运行的 Session。系统状态和 Codex 用量等依赖外部采集的数据可以在内部标记失效；默认查询仍返回最后成功值，不因失效隐式刷新，也不把缓存状态作为用户异常。
...
槽位同步由完全等价的 `chub sync`、`chub -s` 或中文别名显式触发，是独立写操作，同一时刻只执行一次。它先执行两次完整扫描确认候选稳定，再在提交锁内复检固定配置、原槽位和当前绑定均未变化；任一依据变化或写入无法完整确认时放弃本次写入，不提交部分变更。有效映射保持原编号，空槽按数字从小到大补充；候选依次使用当前有效绑定、已有微信任务历史和内部 Session ID 稳定排序。同步不创建、归档或切换 Session；当前绑定失效时只清除绑定。普通状态查询不能隐式改变槽位。
...
状态总览只返回当前受控身份可见的非敏感摘要，不暴露主机名、Session ID、路径、权限、模型、路由或错误原文。首行使用 `Chub · <耗时>` 展示本次现场状态采集耗时：小于 1 秒显示整数 `ms`，达到 1 秒后显示一位小数的 `s` 并去掉多余 `.0`，例如 `10ms`、`1.3s`、`10s`；不再展示 `Active/Limited`。功能受限仍由异常区给出明确原因。正常状态除此之外只展示 Session 列表和 Codex 用量；系统资源、零任务计数、正常更新时间和就绪说明隐藏。异常按序号集中展示，只包括功能受限、资源告警和通知失败。Session 状态由 Sessions 列表表达，Codex 可用性由 Codex 主体表达；缓存不可用、过期、刷新失败或超时等内部数据状态不进入异常区。Busy Session 仅在能从当前可信微信路由确认运行中的快速交互任务时显示有界脱敏摘要；终端或其他入口造成的 Busy 不猜测任务内容。当前单 Owner 场景可汇总其关联的全部 Session；未来支持多个 Owner 时，任务和 Session 快照必须按可信身份边界隔离，不能直接暴露节点全局详情。

任务完成通知和延迟重启结果继续使用独立的精简 Codex 状态快照，不调用或嵌入完整 `chub` 总览，避免把任务通知状态递归带入通知正文。该内部快照只展示关联 Session 所需的用量与槽位状态，也不得隐式补位或触发通知补发。状态总览、Session 维护回执和该精简快照共用 `Weekly … · Tokens …` 格式，不展示 `Codex Usage:`、`Daily tokens` 或日期后缀；Session 维护回执和重启快照先展示 `Active sessions`，Codex 用量固定放在最后。
...
5. 插件继续验证 Chub 响应边界并按 `pass` / `reply` / `handled` 决定原样交付。
...
文本优化与翻译镜像默认关闭，本机配置决定首次默认值；之后可在 Chub 设置页通过受保护接口即时启停，选择写入 Chub 私有状态，不改写本机 YAML。启用后，普通业务任务会镜像到一个由内部角色标识关联的固定翻译 Session。该 Session 使用独立运行目录和 `read-only` 权限，不符合普通微信 Session 配置，因此不会进入首页会话列表、1–9 槽位或成为当前绑定。关闭只阻止新翻译，已排队或执行中的翻译继续完成；再次开启时使用新的翻译 Session。旧 Session 在所属任务及通知完成后尽力归档；失败时保留清理记录，并在后续启动或翻译状态变化时重试。Chub 将 JSON 编码后的原文作为不可信数据交给 Codex，只接受严格的非空“润色”和“English”两段输出，最终由 Chub 组合原文、润色和英文并原路回送。翻译任务使用有界持久化顺序队列，是不影响业务任务的尽力而为辅助能力；只有成功结果需要回送，登记、执行、取消和通知失败均静默记录。`read-only` 只禁止模型修改文件，不能把提示词视为对本机可读内容的强隔离；维护者应在接受该模型信任边界后再启用。
...
- 因当前绑定 Session 忙而拒绝时，只在私有状态中保留最近一条任务正文 10 分钟；用户可使用 `codex retry` 在当前 Session 续提，或使用 `codex new retry`、`新建会话执行`新建并续提。
- `codex new`、`新建会话`以及指定槽位的中英文切换指令可附带本条任务正文，把现有 Session 操作与任务提交合并为一次显式操作。指令和正文之间接受空格或常见中英文标点，只消费边界的一层分隔符并保留正文自身的路径、隐藏文件和 Markdown 前缀；新建或切换失败时不提交，切换并执行只接受 `Available` 目标。归档及待续提指令不接受附带正文；待续提指令误带正文时固定拒绝且不改变原待续任务。
...
快速交互触发 Chub 延迟重启时，任务完成结果必须先进入通知终态。新实例通过本机健康接口确认恢复后，只有微信 Chub 来源任务会按任务保存的账号和发送者再发送一条重启结果；成功文案、关联 Session、关联任务摘要、Codex 用量和 Active sessions 各区块之间保留空行，Active sessions 标题与列表连续展示。状态区块使用独立的精简 Codex 状态快照，包含 `[Current]` 和已分配 Session 状态，不依赖任何用户状态指令，也不补入等待候选。页面来源只更新快速交互时间线。重启结果通知使用独立的至多一次状态，发送中再次中断时不自动重试，失败不改变任务或重启结果，也不切换到全局目标。
