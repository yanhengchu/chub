...
任务计数在查询时读取已与 Worker 对账的本地任务投影，Session 快照由本地 Session 操作更新，并在展示时用当前快速交互运行态覆盖 `Busy`。每次 `chub` 查询并行采集系统状态、当前已分配 Session 和 Codex 用量；并发查询共享正在执行的同一次采集，不设置刷新冷却。启动初始化缓存只用于服务刚启动和采集失败时兜底，不在用户总览中暴露缓存年龄或刷新机制。内存或磁盘达到 85% 时进入总览告警；不使用 CPU 瞬时值触发首版告警。后续调整这些规则时必须同步能力清单和相关测试，不能允许消息正文覆盖。
...
槽位同步由完全等价的 `chub sync`、`chub -s` 或中文别名显式触发，是独立写操作，同一时刻只执行一次。它先执行两次完整扫描确认候选稳定，再在提交锁内复检固定配置、原槽位和当前绑定均未变化；任一依据变化或写入无法完整确认时放弃本次写入，不提交部分变更。有效映射保持原编号，空槽按数字从小到大补充；候选依次使用当前有效绑定、已有微信任务历史和内部 Session ID 稳定排序。同步不创建、归档或切换 Session；当前绑定失效时只清除绑定。普通状态查询不能隐式改变槽位。槽位候选选择与用户可见列表排序相互独立，后者统一按创建时间倒序展示。
...
状态总览只返回当前受控身份可见的非敏感摘要，不暴露主机名、Session ID、路径、权限、模型、路由或错误原文。首行使用 `Chub · <耗时>` 展示本次现场状态采集耗时：小于 1 秒显示整数 `ms`，达到 1 秒后显示一位小数的 `s` 并去掉多余 `.0`，例如 `10ms`、`1.3s`、`10s`；不再展示 `Active/Limited`。功能受限仍由异常区给出明确原因。正常状态除此之外只展示 Session 列表和 Codex 用量；系统资源、零任务计数、正常更新时间和就绪说明隐藏。异常按序号集中展示，只包括功能受限、资源告警和通知失败。Session 状态由 Sessions 列表表达，Codex 可用性由 Codex 主体表达；缓存不可用、过期、刷新失败或超时等内部数据状态不进入异常区。所有 Session 列表按创建时间倒序展示，内部 Session ID 仅作为同一创建时间的稳定兜底；微信列表仍只包含已分配槽位且不改变槽位编号，固定使用 `S<槽位> · 标题` 和独立状态行，当前槽位在状态后追加 `Current`。Busy Session 仅在能从当前可信微信路由确认运行中的快速交互任务时显示有界脱敏摘要；终端或其他入口造成的 Busy 不猜测任务内容。当前单 Owner 场景可汇总其关联的全部 Session；未来支持多个 Owner 时，任务和 Session 快照必须按可信身份边界隔离，不能直接暴露节点全局详情。

任务完成通知和任务级重启结果继续使用独立的精简 Codex 状态快照，不调用或嵌入完整 `chub` 总览，避免把任务通知状态递归带入通知正文。该内部快照只展示关联 Session 所需的用量与槽位状态，也不得隐式补位或触发通知补发。状态总览、Session 维护回执和该精简快照共用 `Weekly … · Tokens …` 格式，不展示 `Codex Usage:`、`Daily tokens` 或日期后缀；Session 维护回执和重启快照先展示 `Sessions`，Codex 用量固定放在最后。
