...
本节是微信 Chub 固定指令的唯一产品契约。消息只有在首字符为空白字符或 Unicode 标点时才进入固定指令匹配；常用英文和中文标点均可作为前缀，例如 `/`、`\`、`,`/`，`、`.`/`。`、`!`/`！`、`?`/`？`、`;`/`；`、`:`/`：`，以及中文顿号 `、`。无此前缀的消息一律进入普通任务流程。进入匹配后，完整匹配的表内指令由固定路由处理，匹配失败保留原文并降级为普通任务。
...
- 指令前缀只看消息首字符：首字符为空白字符或 Unicode 标点才尝试匹配；匹配前移除开头连续空白和标点及其后的分隔空白。无前缀的 `new`、`检查状态`、`help` 等内容均按普通任务发送，不再兼容旧的无前缀指令。Unicode 符号或 Emoji 不自动作为指令前缀。
...
- 指令解析必须先整体判定槽位，不能把连续数字拆成合法前缀再执行；例如带前缀的 `切换S10正文`、`切换10正文` 匹配失败后作为普通任务，不得误解析成 `S1` 加正文 `0正文`。
- `new`、`rename` 的标题不能为空且最多 48 个字符。`switch` 仅在槽位后存在正文时提交任务；`stop`、`archive`、`restart` 和续提指令不接受任意附带正文。
- `cat RN`、`run RN`和`archive RN`不接受附带正文；带前缀但缺失、连续多位、越界或非 R 英文需求槽位时匹配失败并作为普通任务提交。需求标题最多 48 字符，正文最多 2000 字符。
...
| `restart` 的首次 `Scheduled` 回复 | 只返回 Scheduled，不附加 Session/Task 状态和用量 |
| `restart` 已在进行中或同步失败的回复 | 按现有固定指令规则附加可用状态 |
