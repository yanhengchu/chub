...
Chub 是个人本地工作站与统一控制面：核心能力可独立运行，插件模块可在本机安装、版本和启停边界内扩展业务。核心优先保持可用、局部恢复和最终状态确认，不因普通模块维护、单项故障或状态暂时未知增加全局门禁；只有直接数据破坏、安全越界或不可恢复冲突才需要拒绝。“插件模块”是统一产品与架构术语；正式交付和安装形态统一称为“插件包”或“插件 ZIP”。当前 Runtime 插件模块与微信任务编排插件模块已实现；Deliveryline 作为首个工作台业务模块的方向已确认，但业务模块宿主、安装和调用入口尚未实现。
...
| 工作台业务模块 | 已确认方向：首个固定开发模块为 Deliveryline | 将由 Chub 提供固定页面分区、模块专属数据边界和受控能力调用；当前没有宿主、安装、页面或调用入口，ZIP 生命周期待出现第二个真实业务模块后再定义。 |
...
| 帮助清单 | `help` 只显示高频的状态、Session、认证和维护指令：`chub · check · usage · sync · new [title] · S# [task]`、`codex auth · codex auth switch`、`stop [S#] · retry · archive S# · del S#`；下方 `More commands` 保留五个主题入口。所有主题统一使用 `<topic> help`；各主题帮助分别显示本类完整语法，标题统一为 `Commands · <Topic>`；标题与每项均为独立段落 |
