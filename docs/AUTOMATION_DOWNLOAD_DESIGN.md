...
Debug Chrome：运行中 · 默认用户 · 有界面       已启用 1 个任务
...
- 未启动时依次展示浏览器用户选择器、界面选择器和“启动”；默认有界面，便于人工登录和处理验证码，也可明确选择无界面。
...
```text
浏览器环境   [浏览器用户] [有界面/无界面] [启动]

浏览器环境   运行中 · 默认用户 · 有界面       [停止]
```

- 浏览器停止时允许选择用户和界面状态；运行后锁定两项选择，必须停止后才能切换。
- 用户选择、界面状态选择和启动在同一共享浏览器锁内完成，防止任务、飞书检查、Profile 切换和浏览器启停并发交错。
...
每次运行使用新的受控输入快照，不覆盖先前批次；汇总时由 `mapping.json` 明确选择采用的批次。每份文档独立使用单文档超时；某份失败后继续处理剩余文档，主文件和本次已经成功的关联文件不会回滚。全部关联文档成功时任务成功，任一关联文档失败时任务最终标记失败并显示成功数量，页面可展开查看每份文档的结果。主周报中未找到目标章节、没有合法链接或超过数量上限时，主文件仍保留，但扩展阶段和整体任务标记失败。
...
- `directory`：普通任务使用相对于自动化数据根目录的任务下载目录，解析后的路径必须仍位于该根目录内。周报专属任务由 Runner 固定映射到当前本期周期的 `inputs/<下载批次>/`，不接受客户端路径。
