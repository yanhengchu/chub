...
- [ ] 验证 ClawBot 执行首批低风险白名单任务并查询最终结果。
...
- [ ] 按真实稳定性或成本需求完成超时、额度限制和回退模型策略。
- [ ] 验证模型不可用、限流和失败时的降级反馈。

## 5. M5：端到端与安全验收

- [x] 完成飞书群机器人 Webhook 普通消息单向通知链路。
- [ ] 完成首批低风险白名单操作的“微信 ClawBot ↔ OpenClaw/共享基础 LLM ↔ Chub/目标电脑”双向交互链路。
- [ ] 验证请求、处理中状态、最终结果和操作日志可追踪。
- [ ] 验证重复请求、超时、断链和服务不可用路径。
- [ ] 完成首批低风险白名单操作在 macOS、Ubuntu 和手机/微信入口的实际验收。
...
选择 1～2 个真实有价值、参数固定且可恢复的低风险白名单能力，完成“微信 ClawBot → OpenClaw → Chub → 目标状态 → 微信最终回复”的闭环。状态查询已经验收，可作为链路基线，但不再把重复查询验收当作新功能。
...
- LLM 回退模型和额度策略：先保持当前明确失败语义，出现真实稳定性或成本需求后再设计。
- 更多 Chub Tool：必须由具体需求驱动，不以扩充 Tool 数量作为阶段目标。

OpenClaw 与 Chub 共享基础 LLM、`chub_get_status`、微信基础双向消息和飞书通知首版均已收尾，不再列为遗留任务。
