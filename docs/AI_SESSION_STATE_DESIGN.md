...
- 服务重启会结束未完成的快速交互并恢复一致的 Session Activity，不保留失效的
  `working + quick`。
- 首页使用紧凑的两态交互入口切换按钮，不再单独放置“快速交互”和“交互记录”按钮；进入快速交互后，
  再按当前浏览器设置使用任务视图或会话视图。首页入口偏好、快速交互视图偏好和每页 5/10 条的分页偏好
  都属于浏览器 UI 状态，不写入 Session、Activity、Execution 或任务。
- 任务视图和会话视图统一承载提交入口、任务状态、结果和历史记录，共享同一任务和执行逻辑；会话视图只是
  按时间线将相同任务映射为问答消息，不创建新的后端“会话”类型。
- 快速交互页面避免在输入区和交互记录中重复展示同一个执行状态；输入区保留提交和必要的阻断、确认
  信息，任务执行状态由交互记录统一呈现。
- 任务视图输入卡片的折叠属于当前页面的临时展示状态，不进入 Session 模型，也不持久化；进入页面时
  可以根据快速交互是否正在执行选择初始折叠状态，后续任务结束不反向修改用户当前的折叠选择。
- 快速交互轮询按任务 ID 原位更新变化记录，不重建稳定历史；输入卡片与记录列表分别处理刷新失败，
  避免一个区域的临时错误清空或阻断另一区域。
- 快速交互记录的置顶属于任务展示元数据，不改变 Session、Activity、任务时间或执行结果。任务视图保持
  “最近一条、置顶记录、普通记录”三层顺序；会话视图保持按创建时间的时间线顺序。两种排序均由后端在分页前完成。
