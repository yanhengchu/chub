...
- 待重启状态持久化为 `waiting` 或 `started`，并保存发起实例 ID。节点级状态文件继续只描述一次实际重启；
  ...
  仅当旧实例已将状态推进为 `started` 时，新实例才在每个关联任务后发送“Chub 已完成自动重启，服务已恢复。”，并附加关联 Session、关联任务摘要，以及独立精简的状态区块；该区块先展示 `Active sessions`，最后展示 `Weekly … · Tokens …`，且不依赖用户状态指令。
...
- 快速交互页面避免在输入区和交互记录中重复展示同一个执行状态；输入区保留提交和必要的阻断、确认
  信息，任务执行状态由交互记录统一呈现。
- 快速交互输入区上方使用“编号 · 状态”的 Session 导航（例如 `1 · 待输入`、`2 · 执行中`），
  当前 Session 与执行中 Session 分别提供明确的选中态和非纯颜色状态提示；编号按当前浏览器绑定
  Session，多个按钮在窄屏横向滚动。页面切换不停止原 Session 的后台任务，并在当前标签页按 Session
  暂存未发送内容。快速交互页内的普通 Session 切换替换当前浏览器历史项，不叠加返回记录；从首页首次
  进入仍使用普通导航，新标签页操作仍遵循浏览器默认行为。
...
单任务自动延迟重启已在 macOS 完成实机验证，微信 Chub 来源的任务完成结果与新实例恢复后的独立重启结果已完成真实原路回送验收；多任务等待、来源分流、手动清标记、启动失败、状态文件异常及 macOS/Linux 服务管理分支由自动化测试覆盖。
