...
实际阅读页面内容，因此使用短暂宽限而不是等待浏览器确认。首版不设置强制等待上限：只要已有快速交互仍在执行，
...
- 快速交互触发重启时，发起任务的最终结果不会被重启中断；其他执行中任务结束前不会自动重启，待重启期间
  不接受新任务；自动重启成功后原任务显示第二条系统回复，手动重启不显示该回复且不会留下二次自动重启。
...
2026-08-10 已在 macOS 完成单任务自动延迟重启实机验收：

- 快速交互先展示完整任务结果和“即将重启”提示，登记过程未中断当前任务。
- 短暂宽限后服务自动重启，新实例恢复并消费待重启标记。
- 原任务的延迟重启状态由 `pending` 更新为 `succeeded`，完成时间已持久化。
- 快速交互页面跨过服务短暂不可用窗口继续轮询，无需手动刷新即追加固定 Chub 系统回复。
- 操作日志形成 `requested → started → succeeded` 完整链路，最终成功由新实例确认。

多任务并发等待、待处理期间由系统服务管理器手动重启、重复登记、启动失败和状态文件异常均已有自动化测试。
其中并发等待、手动清标记和 Ubuntu 实际服务重启尚未完成本轮实机复验，后续涉及相关平台或场景时回归；
这不影响本次已验收的 macOS 单任务自动重启链路。
