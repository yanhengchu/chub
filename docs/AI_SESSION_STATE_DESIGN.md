...
- 多个登记合并为一次节点级重启。登记任务失败、超时或取消时不提升为待重启，也不会自动重启。
- 任务最终结果由 Chub 追加准确状态：无其他任务时提示“即将重启”，有其他任务时提示当前等待数量；
  登记失败时明确提示本次不会自动重启。
- 待重启状态持久化为 `waiting` 或 `started`，并保存发起实例 ID。旧实例只记录重启已开始；新实例
  启动后根据不同实例 ID 清理状态并记录最终成功，不能把子进程创建成功当作服务重启成功。仅当旧实例已将
  状态推进为 `started` 时，新实例才在原快速交互任务后追加“Chub 已完成自动重启，服务已恢复。”系统回复；
  该回复不调用 Codex 或 LLM。
...
单任务自动延迟重启已在 macOS 完成实机验证，多任务等待、手动清标记、启动失败、状态文件异常及 macOS/Linux 服务管理分支由自动化测试覆盖。后续接入新的 AI CLI 时保留本文状态语义，并将供应方差异限制在适配器内。
