...
> 状态：已验收。
>
> 本文只维护当前已落地的业务架构、状态边界、重启语义和验收基线。历史实施阶段、过程记录与一次性切换步骤不再作为长期设计内容保留。
...
普通 `chub restart` 或首页手动重启只重启 Web 服务。Worker、正在运行的 Runner、翻译队列和实时 tmux Session 保持不变。
...
- `chub worker-cutover` 只用于旧部署首次迁移；已切换环境不再重复执行。
- 一次性迁移允许不支持恢复任务查询的旧 Worker 继续切换，但仅限其健康状态确认无活动或损坏任务、未启用任务执行，且受保护的本地任务目录确认完全为空；任一条件无法确认时仍拒绝切换。
