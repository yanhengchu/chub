...
> 状态：调研中。本文描述目标方案，不代表当前实现；当前行为仍以
...
翻译任务同样迁入 Worker，并在上述模型前增加 `queued`。排队时不持有 Session 执行租约，开始执行时再
原子取得；排队截止时间和执行绝对截止时间分别保存，Web 重启不重置。翻译继续使用独立 Session、
`read-only`、有界顺序队列和成功才通知的现有产品语义，但不再由 Web 线程启动 Codex Runner。
...
- Worker 不可用或新 Web 尚未完成租约恢复时，相关 Session 操作返回暂不可用。
- 原生 writer 在检查与 Runner 启动之间出现竞态时，以 Codex 启动结果为准，任务安全失败，不启动第二次。

约束只作用于冲突 Session；其他 Session 和非 Session 功能不受影响。
...
Worker 不可用时，其他页面和无关功能可以正常提供；快速交互及可能冲突的 Session 操作明确返回 503，
不回退为 Web 进程内执行。
...
- 提交规格：任务 ID、Session ID、受控执行配置、创建时间、绝对截止时间和协议版本。
...
写入使用临时文件加原子替换；任务接受和最终完成在必要落盘成功后才对外确认。进程存在只表示任务可能
仍在运行，不能代替完成记录；Runner 身份不能只依赖可能复用的 PID。
...
- Web 与 Worker 至少兼容当前和相邻协议版本；不兼容时快速交互失败关闭。
...
普通任务不阻塞 Web 重启。只有快速任务明确正在修改 Chub 会立即加载的运行代码、本机配置或依赖时，才
设置任务级 `restart_sensitive` 标记；自动 Web 重启等待这些任务结束，避免加载明显处于修改过程中的文件。
...
- Worker 升级只在没有活动任务时进行；服务定义变化后需要重新安装并分别验证 Web 与 Worker 健康状态。
- 节点 stop、卸载和强制 Worker 升级必须明确处理活动任务，不属于普通 Web 重启流程。
...
- 实现每任务目录、幂等提交、Runner 启动握手、查询、取消、绝对超时和原子完成记录。
...
- 翻译的有界顺序队列、独立 Session、`read-only` 和通知语义保持不变。
...
- 同步迁移翻译重启边界并加入轻量 `restart_sensitive` 保护，避免出现任务类型差异。
