...
## 1. 当前 Codex Runtime 专属边界

Codex 通过 Runtime 共享契约接入 Chub。通用的 Session、Worker、Adapter、Runner、模型目录、权限、错误、任务终态和 `usage_snapshot` 能力语义不在本文重复定义；本文只说明当前 Codex 对这些边界的私有实现事实。
...
账户行以同一次 Runtime 检查同时展示认证方式、5h 与 Weekly 剩余百分比及检查时间：例如“ChatGPT 账户已登录 · 5h 42% · Weekly 78% · 检查于 08-15 10:30”。账户模式为“ChatGPT 账户已登录”，API 模式为“API Key 模式已启用”；额度读取失败时认证结果保持不变，并显示“额度暂不可用”；因 Debug Chrome 未启动无法读取 API 额度时显示“浏览器未启动，额度暂无法获取”。5h 是可选项，缺失时只显示 Weekly；不在账户行展示重置时间、金额或 Token。
...
### 2.2 Sub2API API Key 方式

这不是通用 API Key 平台适配。Chub 复用已启动且已登录的受管 Debug Chrome，并只按固定的 Sub2API 路径和响应字段采集：

- 先创建受管 Chrome 的不聚焦后台页面，并通过 CDP 仅对该页面模拟焦点和活动生命周期，使 provider 自己的页面运行时附加认证信息后请求固定订阅接口。后台页面不出现在前台，采集结束立即关闭。只有页面实际跳转到登录页才视为未登录；固定接口的 `401/403` 不能单独推断登录态，因为 provider 可能由页面运行时附加认证信息。认证失效时不再请求仪表盘。
...
- 刷新检测到认证类型或账号身份变化时不复用旧数据；当前 Codex provider 根地址变化后，下一次用量读取会使用新配置并重建该 Runtime 的采集服务，无需重启 Chub。
...
- Sub2API API Key 方式依赖受管 Chrome 已运行且目标页面已经登录；未登录时仅可通过账户检查明确提供的固定恢复操作进入登录页。
