(() => {
  ...
  window.openWorkspaceQuickSession = (session) => {
    ...
    const currentContent = document.getElementById("workspace-section-content");
    if (!(currentContent instanceof HTMLElement)) {
      window.location.assign(`/ai/sessions/${encodeURIComponent(session.id)}/quick-interactions/conversation`);
      return;
    }
    ...
    frame.src = `/ai/sessions/${encodeURIComponent(session.id)}/quick-interactions/conversation?embedded=workspace`;
