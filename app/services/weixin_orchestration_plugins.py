...
class _Manifest(_StrictModel):
    ...
    def validate_supported(self, settings: Settings) -> None:
        if self.protocol_version != MODULE_PROTOCOL_VERSION:
            raise ValueError("任务编排插件协议版本不受支持。")
        if self.module_type != "capability-orchestration":
            raise ValueError("任务编排插件类型必须为 capability-orchestration。")
        if self.scope != "weixin-normal-text":
            raise ValueError("任务编排插件范围不支持当前微信任务。")
        ...
        if self.chub_version != settings.app.version:
            raise ValueError("任务编排插件与当前 Chub 版本不兼容。")
...
class _Registry(_StrictModel):
    ...
    def validate_supported(self) -> None:
        if self.version != REGISTRY_VERSION:
            raise ValueError("任务编排插件注册表版本不受支持。")
        refs = [item.implementation_ref for item in self.artifacts]
        if len(refs) != len(set(refs)):
            raise ValueError("任务编排插件注册表包含重复产物。")


@dataclass(frozen=True)
class WeixinOrchestrationPluginPreview:
    module_id: str
    version: str
    name: str
    description: str
...
@dataclass(frozen=True)
class WeixinOrchestrationPluginArtifact:
    implementation_ref: str
    module_id: str
    version: str
    name: str
...
class WeixinOrchestrationPluginService:
        ...
        with self._lock:
            ...
            try:
                ...
                if destination.exists():
                    raise self._invalid("任务编排插件产物目录状态无法确认。")
    ...
    def list_artifacts(self) -> tuple[WeixinOrchestrationPluginArtifact, ...]:
        with self._lock:
            registry = self._read_registry()
            listed: list[WeixinOrchestrationPluginArtifact] = []
            for artifact in registry.artifacts:
                try:
                    self._load_registered(artifact)
                except ApiError as exc:
                    listed.append(
                        WeixinOrchestrationPluginArtifact(
    ...
    def require(self, implementation_ref: str | None) -> _LoadedModule:
        if not isinstance(implementation_ref, str):
            raise self._invalid("任务编排插件引用无效。")
        with self._lock:
            registry = self._read_registry()
            artifact = next(
                (
            ...
            if artifact is None:
                raise self._unavailable("已绑定的任务编排插件产物不可用。")
    ...
    def execute_refinement(
        ...
        *,
        ...
        try:
            ...
            raise self._unavailable("任务编排插件执行失败，本次任务未执行。") from exc
    ...
    def remove(self, implementation_ref: str) -> None:
        with self._lock:
            ...
            if artifact is None:
                raise self._invalid("任务编排插件不存在或不可移除。")
            root = self.artifacts_dir / artifact.content_sha256
            if not root.is_dir() or root.is_symlink():
                raise self._unavailable("任务编排插件产物目录不可用，无法确认移除状态。")
    ...
    def _load_registered(self, artifact: _RegistryArtifact) -> _LoadedModule:
        ...
        if (
            ...
            raise self._unavailable("任务编排插件注册信息与产物不一致。")
    ...
    def _validate_archive_input(self, archive: bytes, source_name: str) -> str:
        if not archive or len(archive) > self.settings.openclaw.weixin_chub_mode.orchestration_module_max_archive_bytes:
            raise self._invalid("任务编排插件包无效或超过固定大小上限。")
        safe_name = Path(source_name).name
        if not safe_name.lower().endswith(".zip"):
            raise self._invalid("只接受 ZIP 格式的任务编排插件包。")
        return safe_name
    ...
    def _extract_candidate(self, candidate: Path, archive: bytes) -> Path:
        ...
        try:
            with zipfile.ZipFile(archive_path) as package:
                ...
                for member in members:
                    ...
                    if (
                        ...
                        raise self._invalid("任务编排插件包包含不允许的路径。")
            ...
            raise self._invalid("任务编排插件包不可读取。") from exc
        return content
    ...
    def _read_manifest(self, root: Path) -> _Manifest:
        ...
        try:
            ...
            raise self._unavailable("任务编排插件清单不可读取。") from exc
        if len(raw) > MAX_MANIFEST_BYTES:
            raise self._invalid("任务编排插件清单超过固定大小上限。")
        try:
            manifest = _Manifest.model_validate_json(raw)
            manifest.validate_supported(self.settings)
        except (ValidationError, ValueError) as exc:
            raise self._invalid("任务编排插件清单格式或协议无效。") from exc
        return manifest
    ...
    def _read_registry(self) -> _Registry:
        ...
        try:
            ...
            raise self._unavailable("任务编排插件注册表不可读取。") from exc
        if len(raw) > MAX_MANIFEST_BYTES:
            raise self._unavailable("任务编排插件注册表无效。")
        try:
            registry = _Registry.model_validate_json(raw)
            registry.validate_supported()
        except (ValidationError, ValueError) as exc:
            raise self._unavailable("任务编排插件注册表无效。") from exc
        return registry
    ...
    def _write_registry(self, registry: _Registry) -> None:
        ...
        try:
            ...
            raise self._unavailable("任务编排插件注册表无法保存。") from exc

    def _prepare_root(self) -> None:
        for path in (self.root, self.artifacts_dir, self.staging_dir):
            if path.exists() and path.is_symlink():
                raise self._unavailable("任务编排插件安装目录不可用。")
    ...
    def _load_entry(
        ...
        try:
            ...
            for name in tuple(sys.modules):
                if name == namespace or name.startswith(f"{namespace}."):
                    del sys.modules[name]
            raise self._invalid("任务编排插件入口加载失败。") from exc
    ...
    def _content_hash(self, root: Path) -> str:
        ...
        try:
            ...
            raise self._unavailable("任务编排插件产物不可读取。") from exc
        for path in files:
            if path.is_symlink():
                raise self._unavailable("任务编排插件产物包含不允许的链接。")
            try:
            ...
            if total_bytes > self.settings.openclaw.weixin_chub_mode.orchestration_module_max_archive_bytes:
                raise self._unavailable("任务编排插件产物超过固定大小上限。")
    ...
    def _preview(
        ...
        return WeixinOrchestrationPluginPreview(
    ...
    def _invalid(message: str) -> ApiError:
        return ApiError(422, "weixin_orchestration_plugin_invalid", message)

    @staticmethod
    def _unavailable(message: str) -> ApiError:
        return ApiError(503, "weixin_orchestration_plugin_unavailable", message)
