...
项目启动时会自动读取根目录 `.env`。系统中已经存在的环境变量优先，不会被
`.env` 覆盖。`.env` 只建议配置：

```bash
HUB_TOKEN=replace-with-a-long-random-token
# HUB_CONFIG_FILE=/absolute/path/to/settings.yaml
```

`.env` 已被 Git 忽略，不会提交；`.env.example` 只保留占位内容。macOS 和
Ubuntu 上的真实文件建议限制为仅当前用户可读：
