...
- 在设置页选择新建 Session 的默认权限，支持 `Ask for approval`、`Approve for me`、`Full access` 和 `Read Only`，默认 `Full access`；该偏好只影响当前浏览器之后新建的会话，已有会话不变。
