...
`V 国内业务周报` 的本期周期固定为周一至周日：每周三开始本期处理窗口，至下周二固定报送日结束。周三至周日处理当周周期，下周一、周二继续处理同一周期；自动化下载与两阶段汇总始终归属本期。主周报和“各端周报”关联文档会写入 `data/artifacts/weekly-reports/<周期>/inputs/<下载批次>/`；同周期重跑保留独立批次，不覆盖既有原始材料。各端来源自身可使用周一至周五、周一至周六或周二至周六等不同统计范围，只要 Manifest 中声明的实际使用范围落在本期周一至周日内，即可作为本期材料。扩展只解析“各端周报”章节内同租户的飞书 Wiki 或 Docx 文档链接，去重后串行下载 Markdown。单份关联文档失败不会阻止后续文档，并在任务状态中展示汇总及可展开明细。
...
- `data/state/`：需要在服务重启后恢复的 Codex 会话、快速交互时间线、自动化状态和项目资料归档状态。
