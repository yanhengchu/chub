...
Chub 提供配置驱动的网页下载自动化基础能力，复用独立 Debug Chrome 的登录状态。首页可以启动或停止 Debug Chrome、查看浏览器和任务状态、手动运行任务，并在“全部任务”页面查看完整列表。自动化执行期间不允许停止 Debug Chrome。
...
示例任务默认关闭。按真实网站修改入口、登录检查、步骤和文件校验后再启用。也可以通过统一 Runner 手动执行：

```bash
.venv/bin/python -m app.automations.command run <task-id>
```

Runner 不会自行启动或停止 Debug Chrome。当前尚未接入系统定时器，真实网站流程需要逐项验收后再启用调度。
...
- [配置驱动的网页下载自动化方案](docs/AUTOMATION_DOWNLOAD_DESIGN.md)
