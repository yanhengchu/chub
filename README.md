...
- **微信 Chub 模式**：OpenClaw 在模型调度前把可信微信私聊交给 Chub；固定路由直接回复，普通任务先确认提交，完成后再向原发送者回送结果。
...
OpenClaw 只提供可信入口和通道上下文，Chub 负责路由、安全校验和最终状态。固定路由直接返回结果且不创建 Codex 任务；其他非空消息进入普通任务，先收到提交状态，再在成功、失败或超时后原路收到最终结果。整条链路不调用 OpenClaw Agent，也不把拦截、提交或通知发送等同于任务成功。
